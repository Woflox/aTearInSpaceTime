    ...
    public class Explosion : triangleEntity
    {
        ...
        public static void CreateExplosionAtPoint(Vector2 pos)
        {
            for (int i = 0; i < 20; i++)
            {
                ...
                Game1.instance.entities.Add(new Explosion(pos, Game1.randUnitVector(), 0.1f * size, 0.4f * size, new Color(0, (byte)Game1.rand.Next(64), (byte)Game1.rand.Next(128))));
            }
            Game1.instance.timeSinceExplosion = 0;
        }


        public Explosion(Vector2 pos, Vector2 direction, float width, float length, Color color)
            : base(pos, direction, width, length, color, EntityType.Explosion)
        {
            ...
            if (destroyedShip)
            {
                CreateExplosionAtPoint(shipPos);
            }
